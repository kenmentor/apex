...
export async function POST(request) {
  try {
    ...
    await col.insertOne({ event, payload, createdAt: new Date() })
...
export async function GET() {
  try {
    ...
    const frictionData = await col.aggregate([
      ...
      },
      {
        $project: {
          ...
          frictionIndex: {
            $round: [{ $multiply: [{ $divide: [90, { $min: ['$avgTime', 90] }] }, { $subtract: [1, { $divide: ['$correct', '$total'] }] }] }, 2],
          },
        },
      },
    ...
    const exhaustionData = await col.aggregate([
      ...
      },
      {
        $project: {
          ...
          totalSteps: 1,
          dropoutRate: { $round: [{ $subtract: [1, { $divide: [{ $avg: '$lastStep' }, '$totalSteps'] }] }, 2] },
          sessions: '$count',
        },
      },
      { $sort: { dropoutRate: -1 } },
    ...
    const spaceData = await col.aggregate([
      ...
      { $group: { _id: { space_id: '$payload.space_id', action: '$payload.action' }, count: { $sum: 1 } } },
      { $sort: { '_id.space_id': 1 } },
    ]).toArray()
    const spaceMap = {}
    ...
    const syllabusData = await col.aggregate([
      ...
      { $group: { _id: { course_code: '$payload.course_code', topic_id: '$payload.topic_id' }, count: { $sum: 1 } } },
    ...
    const rageClickData = await col.aggregate([
      ...
      { $group: { _id: '$payload.target_element_selector', count: { $sum: 1 }, coords: { $last: '$payload.click_coordinates' } } },
      { $sort: { count: -1 } },
      { $limit: 20 },
    ]).toArray()
    ...
    const networkData = await col.aggregate([
      ...
      { $project: { endpoint_url: '$_id', count: 1, avgDurationMs: { $round: ['$avgDurationMs', 0] }, maxDurationMs: 1, statuses: 1 } },
    ...
    const validationData = await col.aggregate([
      ...
      { $group: { _id: { field: '$payload.input_field_id', rule: '$payload.failed_rule_type' }, count: { $sum: 1 } } },
      { $sort: { count: -1 } },
      { $limit: 30 },
    ]).toArray()
    ...
    const copyData = await col.aggregate([
      ...
      { $group: { _id: '$payload.view_context', count: { $sum: 1 }, avgLength: { $avg: '$payload.copied_text_length' }, maxLength: { $max: '$payload.copied_text_length' } } },
      { $project: { view_context: '$_id', count: 1, avgLength: { $round: ['$avgLength', 0] }, maxLength: 1 } },
      { $sort: { count: -1 } },
    ]).toArray()
